...
- **Electron 36–41 ARGB regression on X11.** `transparent: true` + `backgroundColor: '#00000000'` causes transparent CSS areas to render as opaque white on Linux X11 in Electron 36–41. Root cause: Chromium's compositing pipeline changed in a way that breaks ARGB visual selection on X11. **Fix: hard-pin to Electron 35.7.5** (see `package.json` `"//"` comment and version guard in `src/main.ts`). Do not upgrade until the regression is confirmed fixed upstream.
